    ...
    <Modal open={!!modal} onClose={() => { setModal(null); setLocalBody(''); }} title={modal === 'add' ? 'Новый сценарий' : 'Редактор'} wide>
      {modal === 'add' ? <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>{saved && <span style={{ fontSize: 13, color: 'var(--accent-green)', fontWeight: 500 }}>✓</span>}<Btn onClick={() => saveScript(s._id)}>Сохранить</Btn><Btn onClick={() => { toggleArchive(s._id, s); setModal(null); }} outline small color="var(--accent-orange)">{s.archived ? '↩ Вернуть' : '📦 В архив'}</Btn><Btn onClick={() => deleteScript(s._id)} color="var(--accent-red)" outline small>Удалить</Btn></div>
        </div>
...
function ConcertsTab({ currentUser }) {
  ...
  const [form, setForm] = useState({ title: '', date: '', time: '', venue: '', city: '', fee: '', status: 'Подтверждено', notes: '', contactName: '', contactPhone: '' });
  ...
  const calDays = []; for (let i = 0; i < startDow; i++) calDays.push(null); for (let d = 1; d <= daysInMonth; d++) calDays.push(d);
  const getForDay = (d) => { const ds = `${year}-${String(month + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`; return list.filter(c => c.date === ds); };
